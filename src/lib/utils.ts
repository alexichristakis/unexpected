...
export const scaleRest = (value: Animated.Value<number>) => {
  return cond(
...
export const scaleFriction = (
  value: Animated.Value<number>,
  rest: Animated.Node<number> | Animated.Node<number>[],
  delta: Animated.Node<number> | Animated.Node<number>[]
) => {
...
export const scaleDiff = (value: Animated.Value<number>) => {
  const tmp = new Value(1);
  const prev = new Value(1);
  return [set(tmp, divide(value, prev)), set(prev, value), tmp];
};

export const dragDiff = (
  value: Animated.Value<number>,
  updating: Animated.Node<0 | 1>
) => {
...
// to `MAX_VALUE`
export const friction = (value: Animated.Node<number>) => {
...
export const speed = (value: Animated.Value<number>) => {
...
export const runTiming = (
  ...
  return [
    cond(clockRunning(clock), 0, [
      ...
      startStopClock ? startClock(clock) : 0
    ]),
    timing(clock, state, config),
    cond(state.finished, startStopClock ? stopClock(clock) : 0),
    state.position
  ];
};
...
export const bouncy = (
  ...
  gestureActive: Animated.Node<0 | 1>,
